...
fig, ax = score_resolutions(
    adata, columns, tests="SH_CH_DB", method="mean", return_plot=True
)
fig.savefig("score_resolutions.png")

# to pre-select promising clustering resolutions, use:
tree_columns, fig, ax = clustering_plot(
    adata, columns, method="score", min_n_resolutions=1, return_plot=True
)
...
os.makedirs("figures/dotplot_", exist_ok=True)
